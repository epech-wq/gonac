...
export default function OportunidadesView({ data }: OportunidadesViewProps) {
  ...
  const getCardIcon = (type: OpportunityType) => {
    switch (type) {
      case 'agotado':
        return (
  ...
  const renderOpportunityCard = (type: OpportunityType, opportunityData: any) => (
    <div key={type} className="rounded-lg bg-white shadow-sm dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      {/* Card Header */}
      <div className={`p-6 bg-gradient-to-r ${getCardColor(type)} text-white rounded-t-lg`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            {getCardIcon(type)}
            <div>
              <h3 className="text-xl font-bold">{getCardTitle(type)}</h3>
              <p className="text-sm opacity-90 mt-1">{getCardDescription(type)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <div className="text-sm text-gray-500 dark:text-gray-400">Impacto Total</div>
            <div className="text-2xl font-bold text-gray-900 dark:text-white">
              {formatCurrency(opportunityData.impacto)}
            </div>
          </div>
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <div className="text-sm text-gray-500 dark:text-gray-400">Tiendas Afectadas</div>
            <div className="text-2xl font-bold text-gray-900 dark:text-white">
